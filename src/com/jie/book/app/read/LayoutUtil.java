...
public class LayoutUtil {
	...
	public static int breakText(StringBuilder contentBuf, int startIndex, int endIndex, int renderWidth, Paint paint) {
		// System.out.println("=startIndex===="+startIndex+"==endIndex==="+endIndex+"===renderWidth=="+renderWidth);
		int suggestCharCount = paint.breakText(contentBuf, startIndex, endIndex, true, renderWidth, null);
		...
		if (lineEndIndex < contentBuf.length()) {
			...
			if (isPunctuation(nextChar)) {
				while (--lineEndIndex > startIndex) { // at least one character
														// will be left
					char ch = contentBuf.charAt(lineEndIndex);
					if (!isSBCPunctuation(ch) && !isASCII(ch)) {
				...
				while (--lineEndIndex > startIndex) { // at least one character
														// will be left
					if (!isAlphanumeric(contentBuf.charAt(lineEndIndex))) {
	...
	public static void layoutTextJustified(CharSequence text, int maxWidth, Paint paint, float startX, float constantY,
			float[] positions) {
		...
		// For SBC_CASE_SPACE_CHARs, I don't increase their kerning, because in
		// chinese text,
